...
int main(int argc, char *argv[])
{
    ...
    // CORREÇÃO DA SENHA: Permite que a janela receba foco do teclado para você conseguir digitar nela
    ...
    // Adiciona os eventos de mouse na janela
        ...
        if (wifi_active)
        {
            ...
            if (!networks.empty())
            {
                for (const auto &net : networks)
                {
                    ...
                    // Evento de clique na rede (Ajustado para gerenciar tamanho e exibição)
                    net_btn->signal_clicked().connect([net_btn, net, rows, action_box, btn_action, entry_password, btn_submit_pass, is_current, &main_box, &window]() {
                        ...
                        // Move a caixa de ação para logo abaixo do botão clicado
                        ...
                        // Mostra apenas o action_box e o btn_action (deixando senha oculta até clicar em Conectar)
                        action_box->show();
                        btn_action->show();
                        entry_password->hide();
                        btn_submit_pass->hide();

                        // Força a janela a recalcular o tamanho imediatamente sem bugar
                        window.resize(1, 1);
                    });
                }
                ...
                btn_action->signal_clicked().connect([btn_action, entry_password, btn_submit_pass, rows, &window]() {
                    ...
                    if (btn_action->get_label() == "Desconectar") {
                        ...
                        if (network_needs_password(selected_ssid)) {
                            ...
                            // Força o ajuste do tamanho da janela para o campo de senha aparecer
                            window.resize(1, 1);
                        } else {
                ...
                // Ação de confirmar a senha (Botão ✓) - Sem fechar se errar
                auto connect_with_pass = [entry_password, rows]() {
                    ...
                    if (!selected_ssid.empty() && !password.empty()) {
                        // Removeu-se o '&' para o comando rodar de forma síncrona (esperar o resultado)
                        std::string cmd = "/usr/bin/nmcli device wifi connect '" + selected_ssid + "' password '" + password + "'";
                        int status = std::system(cmd.c_str());
                        
                        // No Linux, se o comando retorna 0, significa que conectou com sucesso!
                        if (status == 0) {
                            Gtk::Main::quit();
                        } else {
                            // Se falhar (senha errada ou timeout), dá um feedback visual limpando o texto
                            entry_password->set_text("");
                            entry_password->set_placeholder_text("Senha incorreta! Tente novamente.");
                        }
                    }
                };
    ...
    // CORREÇÃO DO MOUSE: Fecha apenas se o mouse sair totalmente da janela
    ...
    return false; });
    // Fechar ao perder foco (caso clique fora)
